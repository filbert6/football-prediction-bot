...
def _parse_seasons(value: Optional[str] | List[int]) -> Optional[List[int]]:
    ...
    for item in [piece.strip() for piece in str(value).split(",") if piece.strip()]:
        try:
            ...
            logger.warning("Skipping invalid Understat season value: %s", item)
...
def _normalize_schedule(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["league"] = df["league"].fillna("Understat")
    df["source"] = "understat"
...
def _get_source_config() -> dict:
    config = load_config()
    return get_data_sources(config).get("understat", {})


def run() -> bool:
    source_config = _get_source_config()
    if not source_config.get("enabled", True):
        logger.info("Understat scraper disabled in configuration")
        return False

    leagues = _make_list(os.environ.get("UNDERSTAT_LEAGUES") or source_config.get("leagues"))
    seasons = _parse_seasons(os.environ.get("UNDERSTAT_SEASONS") or source_config.get("seasons"))
    headless = source_config.get("headless", True)

    if leagues == ["all"]:
        leagues = None

    logger.info("Starting Understat scraper: leagues=%s seasons=%s", leagues, seasons)
    ...
    try:
        reader = Understat(
            ...
            no_cache=False,
            no_store=False,
            data_dir=RAW_DIR / "understat_cache",
        )
        ...
        if schedule is not None and not schedule.empty:
            ...
            if not normalized.empty:
                _save_dataframe(normalized, "understat_schedule.csv", PROCESSED_DIR)

        team_match_stats = reader.read_team_match_stats(force_cache=False)
        _save_dataframe(team_match_stats, "understat_team_match_stats.csv", RAW_DIR)

        shot_events = reader.read_shot_events()
        _save_dataframe(shot_events, "understat_shot_events.csv", RAW_DIR)

        leagues_df = reader.read_leagues()
        _save_dataframe(leagues_df, "understat_leagues.csv", RAW_DIR)

        seasons_df = reader.read_seasons()
        _save_dataframe(seasons_df, "understat_seasons.csv", RAW_DIR)

        logger.info("Understat scraper completed successfully")
        return True
    except Exception:
        logger.exception("Understat scraper encountered an error")
        return False
